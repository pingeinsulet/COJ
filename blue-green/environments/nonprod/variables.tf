# Change this to deploy to a different AWS account; then update vpc_id, subnets,
# alb_acm_certificate_arn, and jenkins_agent_secret_name to match that account.
...
variable "prefix" {
  type        = string
  description = "Prefix for resource names (e.g. jenkins-blue-green-nonprod)"
}
...
variable "route53_zone_id" {
  type        = string
  description = "Route53 hosted zone ID (empty for no alias)"
  default     = ""
}

variable "route53_alias_name" {
  type        = string
  description = "Route53 alias name (e.g. jenkins.nonprod). Empty for no alias."
  default     = ""
}
...
variable "jenkins_webserver_hostname" {
  type        = string
  description = "Hostname for Jenkins UI (e.g. jenkins.nonprod.insulet.com). Set as WEBSERVER in Jenkins global env for jobs."
  default     = ""
}
